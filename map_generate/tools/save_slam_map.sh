...
if [[ -f "$MG_ROOT/install/setup.bash" ]]; then
  # shellcheck source=/dev/null
  source "$MG_ROOT/install/setup.bash"
fi
...
if ! ros2 topic list 2>/dev/null | grep -q '^/map$'; then
  echo "[save] /map not found — keep t2 (slam_mapping) running" >&2
  exit 1
fi
...
echo "[save] done (previous maps untouched):"
