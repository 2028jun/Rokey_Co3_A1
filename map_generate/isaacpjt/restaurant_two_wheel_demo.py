"""Isaac Sim 5.1 restaurant + two-wheel Ridgeback bridge for map_generate.
...
HEADLESS = os.environ.get("MAP_GEN_HEADLESS", "0") == "1"
...
WORKSPACE = Path(
    os.environ.get("MAP_GEN_WS", Path(__file__).resolve().parents[1])
).resolve()
...
def suppress_usd_ros_clock_graphs(stage) -> None:
    """Drop USD/OmniGraph /clock publishers.

    Restaurant timeline loops; Isaac ReadSimulationTime jumps backwards and
    fights the monotonic Python /clock (TF_OLD_DATA + RViz 'No map received').
    """
    graph_names = {
        "EmbeddedSensorsROS2",
        "NavSensorsROS2",
    ...
    for prim in stage.Traverse():
        ...
        elif name == "PublishClock":
            parent = prim.GetParent()
            if parent and parent.IsValid():
                to_remove.append(str(parent.GetPath()))
    for path in sorted(set(to_remove), key=lambda p: -p.count("/")):
        if stage.GetPrimAtPath(path).IsValid():
            stage.RemovePrim(path)
            print(f"[map_generate] removed conflicting ROS graph {path}", flush=True)
...
def configure_wheel_contact_material(stage):
    ...
    print(f"[map_generate] contact materials ready: {bound}", flush=True)
...
class DiffNavBridge(Node):
    ...
    def tick(self, sim_time_sec: float):
        ...
        if (abs(vx) > 1e-3 or abs(wz) > 1e-3) and now - self._last_cmd_log_time > 1.0:
            ...
            print(
                f"[map_generate] cmd vx={vx:.3f} wz={wz:.3f}",
                flush=True,
            )
...
def main():
    ...
    suppress_usd_ros_clock_graphs(stage)
    ...
    print(f"[map_generate] PhysX raycast {RAW_SCAN_TOPIC} ready", flush=True)
    ...
    print(
        f"[map_generate] 2-wheel bridge running; "
