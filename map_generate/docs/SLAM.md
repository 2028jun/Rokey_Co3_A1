...
| 외곽 벽 레인 (±WALL≈3.85) | 벽·의자 사이 통로 |
| 화분 모서리 (NW/NE/SW/SE) | 코너 화분(~±5.1) 앞 look-around로 회색 제거 |
| 남쪽 개구부 앞 | 반대쪽 출입 영역 |

상수(`DOCK`, `OUTER`, `WALL`, `FLANK`, `DOOR_Y`, `KIT_Y`)는 씬 치수에 맞춰 `slam_patrol.py` 상단에서 조정한다.
...
| `WALL` | 3.85 m | 외벽/화분 앞 접근 (화분은 ~±5.1) |
