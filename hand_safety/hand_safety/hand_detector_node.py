...
class HandDetectorNode(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            ...
            f"device: {self.device}, FP16: {self.half}"
        )
    ...
    def process_latest_frame(self) -> None:
        ...
        try:
            ...
            if self.publish_annotated_image:
                try:
                    annotated_message = self.bgr_frame_to_image_message(
                        frame, message.header
                    )
                    self.image_publisher.publish(annotated_message)
                except Exception as exc:  # Debug output must not stop safety.
                    self.warn_throttled(
                        "annotated_image_publish_failed",
                        "Annotated image publication failed; safety state "
    ...
    def bgr_frame_to_image_message(frame: Any, header: Any) -> Image:
        """Build bgr8 Image without cv_bridge's broken CV_8UC3 lookup."""
        ...
        image_message.height = int(bgr_frame.shape[0])
        image_message.width = int(bgr_frame.shape[1])
        image_message.encoding = "bgr8"
        image_message.is_bigendian = False
        image_message.step = int(bgr_frame.strides[0])
        image_message.data = bgr_frame.tobytes()
