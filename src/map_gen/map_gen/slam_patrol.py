...
class SlamPatrol(Node):
    def __init__(self):
        ...
        # Publish directly to "cmd_vel_safe" (what NavBridge actually
        # subscribes to), bypassing nav2_collision_monitor.  Mapping is a
        # supervised, person-free patrol; collision_monitor's stop/slowdown
        # zones are tuned for live serving and false-trigger on doorway
        # frames and walls, silently withholding all output near pinch
        # points (e.g. the kitchen doorway spawn) instead of publishing.
        self._cmd_pub = self.create_publisher(Twist, "/cmd_vel_safe", qos)
