...
class RestaurantMap {
    ...
    render() {
        ...
        if (this.obstaclePerson && this.obstaclePerson.active) {
            ...
            ctx.font = "bold 11px Outfit";
            ctx.textAlign = "center";
            const detectorLabel = this.obstaclePerson.robot
                ? ` · ${this.obstaclePerson.robot.toUpperCase()}`
                : "";
            ctx.fillText(`⚠️ PERSON${detectorLabel}`, oPos.x, oPos.y - 22);
        }
    ...
    setObstaclePerson(active, x = 0.0, y = 2.8, robot = null) {
        this.obstaclePerson = {
            active: !!active,
            x: Number.isFinite(Number(x)) ? Number(x) : 0.0,
            y: Number.isFinite(Number(y)) ? Number(y) : 2.8,
            robot: robot,
        };
        this.render();
    }
