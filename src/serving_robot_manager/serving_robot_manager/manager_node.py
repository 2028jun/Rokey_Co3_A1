...
class ManagerNode(Node):
    ...
    def _on_order_request(self, request, response):
        if self._state == _State.FAILED:
            self.get_logger().warn('FAILED 상태입니다. /manager/reset_fault 호출 후 다시 주문해주세요.')
        ...
        if not (TABLE_ID_MIN <= request.table_id <= TABLE_ID_MAX) or any(
            ...
            self.get_logger().warn(f'잘못된 주문 형식: table={request.table_id}, {counts}')
            response.success = False
            return response
        if not (0 <= request.cutlery_count <= CUTLERY_COUNT_MAX):
            self.get_logger().warn(
                f'식기리필은 0 또는 1만 가능합니다 (받은 값: {request.cutlery_count}).')
            response.success = False
            return response

        if not any(list(counts.values()) + [request.cutlery_count]):
            self.get_logger().warn('서빙할 항목이 없는 주문입니다.')
            response.success = False
            return response
        ...
        if len(serve_queue) > MAX_TRIPS_PER_ORDER:
            self.get_logger().warn(
                f'트립 수가 너무 많은 주문입니다 ({len(serve_queue)}건, 최대 {MAX_TRIPS_PER_ORDER}건).')
            response.success = False
            return response
        ...
        if not (self._arm_client.service_is_ready()
            ...
            self.get_logger().error('Arm/Navigation/음식 스폰 서비스가 준비되지 않아 주문을 거부합니다.')
            response.success = False
            return response
        ...
        if len(self._pending_orders) >= MAX_PENDING_ORDERS:
            self.get_logger().warn(f'대기열이 가득 찼습니다 (최대 {MAX_PENDING_ORDERS}건).')
            response.success = False
            return response
        self._pending_orders.append((request.table_id, serve_queue))
        self.get_logger().info(
            f'다른 주문 처리 중이라 대기열에 추가: table={request.table_id}, '
    ...
    def _call_nav_command(self, command):
        # 이전 도착 판정에 쓰인 캐시가 새 목적지 판정에 섞이지 않도록 보낼 때마다 지운다.
        self._nav_status = None
        self._nav_location = None
        self._nav_moving_confirmed = False
        self._nav_command_accepted = False
        ...
        try:
            ...
            self.get_logger().error(f'Navigation 명령 전송 실패: command={command}, {exc}')
            self._fail()
            return
    ...
    def _on_nav_command_response(
        ...
        if (generation != self._task_generation
            ...
            self.get_logger().warn(f'지연된 Navigation 응답 무시: command={command}')
            return
        if error is not None:
            self.get_logger().error(f'Navigation 명령 처리 중 오류: command={command}, {error}')
            self._fail()
            return
        ...
        self.get_logger().info(f'Navigation 명령 수락: command={command}')
    ...
    def _call_arm_command(self, command):
        ...
        try:
            ...
            self.get_logger().error(f'Arm 명령 전송 실패: command={command}, {exc}')
            self._fail()
            return
        future.add_done_callback(
    ...
    def _on_arm_command_response(self, future, command, generation, command_epoch):
        ...
        if (generation != self._task_generation
            ...
            self.get_logger().warn(f'지연된 Arm 응답 무시: command={command}')
            return
        if error is not None:
            self.get_logger().error(f'Arm 명령 처리 중 오류: command={command}, {error}')
            self._fail()
            return
        if not response.success:
        ...
        self.get_logger().info(f'Arm 명령 수락: command={command}')
    ...
    def _call_spawn_command(self, command):
        ...
        try:
            ...
            self.get_logger().error(f'음식 스폰 명령 전송 실패: command={command}, {exc}')
            self._fail()
            return
        future.add_done_callback(
    ...
    def _on_spawn_command_response(self, future, command, generation, command_epoch):
        ...
        if (generation != self._task_generation
            ...
            self.get_logger().warn(f'지연된 음식 스폰 응답 무시: command={command}')
            return
        if error is not None:
            self.get_logger().error(f'음식 스폰 명령 처리 중 오류: command={command}, {error}')
            self._fail()
            return
        ...
        if not response.success:
            ...
            self._fail()
            return
        # success는 '수락'만 의미한다. 실제 생성 완료는 /food_spawn/status로만 확인하고,
        # 그 전까지는 테이블로 출발하지 않는다.
        self.get_logger().info(f'음식 스폰 명령 수락, 생성 완료 대기: command={command}')
    ...
    def _on_nav_status(self, msg):
        ...
        if self._nav_status == NAV_STATUS_MOVING:
            if self._state in (_State.RETURNING_TO_KITCHEN, _State.MOVING_TO_TABLE):
                # 동일한 MOVING 반복 발행이 멈춘 작업의 타임아웃을 영원히 연장하지 않게
                # 실제 상태 진입을 처음 확인했을 때만 한 차례 갱신한다.
        ...
        if self._nav_status == NAV_STATUS_FAILED:
            if self._state in (_State.RETURNING_TO_KITCHEN, _State.MOVING_TO_TABLE):
                ...
                self.get_logger().warn(
                    f'{self._state.name} 상태에서 Navigation FAILED 상태를 수신했습니다 (무시).')
            return
        self._check_table_arrival()
        self._check_kitchen_arrival()
    ...
    def _check_table_arrival(self):
        # MOVING을 먼저 관찰하고, status=ARRIVED이며 current_location이 요청한 테이블과
        # 같을 때만 도착으로 판정한다 (스펙 3장 + 지연/중복 메시지 방지).
        if (self._state == _State.MOVING_TO_TABLE
            ...
            self.get_logger().info(f'테이블 {self._table_id} 도착 확인, 서빙 시작')
    ...
    def _on_spawn_status(self, msg):
        ...
        if status == SPAWN_STATUS_FAILED:
            if self._state == _State.SPAWNING:
                ...
                self.get_logger().warn(
                    f'{self._state.name} 상태에서 음식 스폰 FAILED 상태를 수신했습니다 (무시).')
            return
        self._check_spawn_completed()
    ...
    def _check_spawn_completed(self):
        if (self._state == _State.SPAWNING
            ...
            self.get_logger().info('음식 스폰 완료 확인, 테이블로 출발')
    ...
    def _begin_arm_serving(self):
        if not self._hand_safety_is_fresh():
            self.get_logger().error(
                '최근 손 침입 감지 신호가 없어 로봇팔을 안전하게 시작할 수 없습니다.')
            self._fail()
            return
        # 서빙을 보내기 전에 손이 이미 ROI에 있는지 확인해, 있으면 명령 자체를 보내지
        # 않고 대기한다 (보낸 뒤 바로 99를 또 보내는 대신, 애초에 시작하지 않음).
        if self._hand_intrusion:
            self.get_logger().warn('ROI 손 침입 상태라 서빙을 대기합니다.')
    ...
    def _on_arm_status(self, msg):
        ...
        if arm_status == ARM_STATUS_FAILED:
            if self._state in (_State.ARM_SERVING, _State.ARM_PAUSED):
                ...
                self.get_logger().warn(
                    f'{self._state.name} 상태에서 Arm FAILED 상태를 수신했습니다 (무시).')
            return
        self._check_arm_completed()
