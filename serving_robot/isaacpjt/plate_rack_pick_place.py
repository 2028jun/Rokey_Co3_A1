...
class PlateRackPickPlace(SodaCanPickPlace):
    ...
    def _follow_transport_tray(self):
        ...
        if not follow_plate_rack_transport(self._stage):
            self.failed = True
            print(
    ...
    def _release_transport(self):
        ...
        rack = self._stage.GetPrimAtPath(PLATE_RACK_PATH)
        ...
        for prim in self._stage.Traverse():
            if not str(prim.GetPath()).startswith(f"{PLATE_RACK_PATH}/"):
                continue
        ...
        handle = self._stage.GetPrimAtPath(f"{PLATE_RACK_PATH}/Handle")
    ...
    def initialize(self, articulation, dof_names):
        ...
        self._set_vertical_orientation_from_base(arm_orientation)
    ...
    def _quaternion_multiply(left, right):
        """Hamilton product for scalar-first USD/Isaac quaternions."""
    ...
    def _set_vertical_orientation_from_base(self, base_orientation):
        """Keep the downward gripper orientation fixed in the robot frame."""
        orientation = self._quaternion_multiply(
            base_orientation, VERTICAL_EE_ORIENTATION
        )
        self._vertical_orientation = orientation / np.linalg.norm(orientation)
