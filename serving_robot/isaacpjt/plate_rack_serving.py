...
def follow_plate_rack_transport(stage):
    ...
    """
    rack = stage.GetPrimAtPath(PLATE_RACK_PATH)
    if not rack.IsValid():
        return False
    tray = find_serving_robot_prim(stage, "upper_tray_left_link")
...
def spawn_plate_rack(stage, plate_count=4):
    ...
    root = UsdGeom.Xform.Define(stage, PLATE_RACK_PATH)
    ...
    base = UsdGeom.Cube.Define(stage, f"{PLATE_RACK_PATH}/Base")
    ...
    for index, default_offset_x in enumerate(PLATE_X_OFFSETS):
        plate = UsdGeom.Cylinder.Define(
            stage, f"{PLATE_RACK_PATH}/Plate_{index + 1:02d}"
        )
    ...
    stem = UsdGeom.Cube.Define(stage, f"{PLATE_RACK_PATH}/HandleStem")
    ...
    handle = UsdGeom.Cube.Define(stage, f"{PLATE_RACK_PATH}/Handle")
    ...
    for index, offset_x in enumerate((-0.1225, -0.0875, -0.0525, 0.0525, 0.0875, 0.1225)):
        divider = UsdGeom.Cube.Define(
            stage, f"{PLATE_RACK_PATH}/Divider_{index + 1:02d}"
        )
