...
# IK-derived safe start for the notched-tray pickup.  link_6 begins at roughly
# [0.05, 0.566, 0.092] in the arm frame with the RG2 already horizontal and
# outside +Y, avoiding the large wrist flip from the former folded pose.
...
TOP_DECK_DISH_LOCAL = np.array([0.38, 0.0, 0.06132])
...
TABLE_DISH_LOCAL = np.array([-0.65, 0.0, -0.06568])
# RG2-specific wooden pizza board.  The 39 cm round deck carries the 32.4 cm
# pizza, while a short world +Y handle gives the gripper a clear grasp zone.
BOARD_RADIUS = 0.195
BOARD_THICKNESS = 0.018
BOARD_HANDLE_WIDTH = 0.060
BOARD_HANDLE_END_Y = 0.265
# Grip farther toward the +Y handle tip so the eye-in-hand D455 housing clears
# the circular deck during the +X insertion and lift.
BOARD_HANDLE_GRASP_Y = 0.220
BOARD_HANDLE_CROSS_INSERT_X = 0.030
# Temporary near-weightless lift test.  PhysX mass=0 can be interpreted as an
# immovable/infinite-mass body, so use 1 gram instead.
BOARD_TEST_MASS = 0.001
# Form-locking channel: fingers enter along +X and close on the central block;
# tall ±Y walls prevent the board from rolling around the X insertion axis.
BOARD_GRIP_BLOCK_CENTER_X = BOARD_HANDLE_CROSS_INSERT_X
BOARD_GRIP_BLOCK_SIZE = np.array([0.130, 0.070, 0.026])
BOARD_SLOT_WALL_SIZE = np.array([0.130, 0.012, 0.040])
BOARD_SLOT_CLEAR_WIDTH = BOARD_GRIP_BLOCK_SIZE[1]
# Raised lips above and below the pad make an 85 mm-long finger pocket.  Once
# closed, the RG2 pads are retained in X by shape instead of friction alone.
BOARD_FINGER_POCKET_LENGTH = 0.085
BOARD_FINGER_LIP_SIZE = np.array([0.012, 0.070, 0.012])
PIZZA_ASSET_SCALE = 0.72
DISH_RADIUS = BOARD_RADIUS
# DynamicCylinder is only the rigid-body carrier; its collider is disabled.
# The visible board mesh and raised grip block provide the actual collisions.
...
def author_wooden_pizza_board(stage, mesh_path, physics_material):
    """Create one watertight round board + short handle mesh in metres."""
    half_handle = BOARD_HANDLE_WIDTH * 0.5
    join_y = float(np.sqrt(BOARD_RADIUS**2 - half_handle**2))
    join_angle = float(np.arctan2(join_y, half_handle))

    # Traverse the long circular edge from the left handle root around the
    # board to the right root, then close it with a rounded +Y handle end.
    circle_angles = np.linspace(
        np.pi - join_angle,
        2.0 * np.pi + join_angle,
        56,
    )
    outline = [
    ...
    outline.extend(
        [
            (
                half_handle * np.cos(angle),
                BOARD_HANDLE_END_Y + half_handle * np.sin(angle),
            )
            for angle in np.linspace(0.0, np.pi, 12)
        ]
    )
    ...
    # Create an actual form-locking channel: a central pad for top/bottom
    # clamping plus tall rails on both Y sides.  When the board tries to roll
    # about X, a rail bears against the side of an RG2 finger instead of
    # relying solely on surface friction.
    def author_lock_box(name, position, size, color):
        root_path = f"{mesh_path}_Lock/{name}"
        ...
        cube_physx = PhysxSchema.PhysxCollisionAPI.Apply(cube.GetPrim())
        cube_physx.CreateContactOffsetAttr(0.001)
        cube_physx.CreateRestOffsetAttr(0.0)

    author_lock_box(
        "GripPad",
        np.array(
            [BOARD_GRIP_BLOCK_CENTER_X, BOARD_HANDLE_GRASP_Y, 0.0]
        ),
        BOARD_GRIP_BLOCK_SIZE,
        (0.48, 0.25, 0.10),
    )
    wall_offset = 0.5 * (BOARD_SLOT_CLEAR_WIDTH + BOARD_SLOT_WALL_SIZE[1])
    for side, sign in (("MinusY", -1.0), ("PlusY", 1.0)):
        author_lock_box(
            f"Rail_{side}",
            np.array(
                [
                    BOARD_GRIP_BLOCK_CENTER_X,
                    BOARD_HANDLE_GRASP_Y + sign * wall_offset,
                    0.0,
                ]
            ),
            BOARD_SLOT_WALL_SIZE,
            (0.40, 0.19, 0.07),
        )
    # Four retaining lips create matching upper/lower finger pockets.  The
    # open fingers pass above and below them during +X insertion; after the
    # gripper closes, the lips resist both +X and -X slip mechanically.
    lip_x_offset = 0.5 * (
        BOARD_FINGER_POCKET_LENGTH + BOARD_FINGER_LIP_SIZE[0]
    )
    lip_z_offset = 0.5 * (
        BOARD_GRIP_BLOCK_SIZE[2] + BOARD_FINGER_LIP_SIZE[2]
    )
    for x_side, x_sign in (("Back", -1.0), ("Front", 1.0)):
        for z_side, z_sign in (("Bottom", -1.0), ("Top", 1.0)):
            author_lock_box(
                f"FingerLip_{x_side}_{z_side}",
                np.array(
                    [
                        BOARD_GRIP_BLOCK_CENTER_X + x_sign * lip_x_offset,
                        BOARD_HANDLE_GRASP_Y,
                        z_sign * lip_z_offset,
                    ]
                ),
                BOARD_FINGER_LIP_SIZE,
                (0.36, 0.16, 0.055),
            )
    print(
        f"[serving] authored wooden pizza board radius={BOARD_RADIUS:.3f}m "
        f"handle=+Y protrusion="
        f"{BOARD_HANDLE_END_Y + BOARD_HANDLE_WIDTH * 0.5 - BOARD_RADIUS:.3f}m "
        f"form_lock_channel={BOARD_SLOT_CLEAR_WIDTH:.3f}m "
        f"finger_pocket={BOARD_FINGER_POCKET_LENGTH:.3f}m "
        f"rails={np.round(BOARD_SLOT_WALL_SIZE, 3)}m "
...
def configure_joint_drives(stage):
    ...
    grip_links = 0
    for prim in stage.Traverse():
        # Imported RG2 collision meshes live below instance proxies and cannot
        # be authored directly through Stage.Traverse().  Bind to each moving
        # link instead; strongerThanDescendants makes the physics-purpose
        # material inherit into its internal collision mesh.
...
def initialize_robot(articulation_path):
    ...
    timeline.play()
    # The board's form-lock channel adds compound collision cooking.  Allow a
    # few frames for PhysX to finish constructing the full articulation before
...
class TrayPizzaPickPlace:
    """Horizontal handle grasp from the open top deck to a table stand."""
    ...
    def __init__(self, stage):
        ...
        # Outside direction for the handle is world -X.  RMP targets move from
        # this side toward +X, along the RG2's physical forward axis.
    ...
    def _enter_phase(self, phase, articulation):
        ...
        self._settled_steps = 0
        names = [
            "move to -X side at handle height",
            "settle outside shortened handle",
            "horizontal insert around board handle",
            "close gripper",
            "vertical lift test",
        ...
        print(f"[serving-horizontal] phase={phase} {names[phase]}", flush=True)
        if phase == 3:
            ...
            if board_lift >= 0.08:
                self.done = True
                print(
                    "[serving-horizontal] vertical lift test complete; "
                ...
                self.failed = True
                print(
                    "[serving-horizontal] VERTICAL LIFT FAILED: wrist rose but "
            ...
            self.done = True
            print("[serving-horizontal] pizza delivered to TableSet_00", flush=True)
    ...
    def initialize(self, articulation, dof_names):
        ...
        pick_grasp = (
            pick_center
            + self._handle_world * BOARD_HANDLE_GRASP_Y
            - self._approach_world * BOARD_HANDLE_CROSS_INSERT_X
            + self._up_world * DISH_GRASP_HEIGHT_OFFSET
        )
        place_grasp = (
            place_center
            + self._handle_world * BOARD_HANDLE_GRASP_Y
        ...
        pick_safe = pick_outside + self._up_world * TOP_DECK_SAFE_CLEARANCE
        pick_vertical = pick_grasp + self._up_world * TOP_DECK_SAFE_CLEARANCE
        # Start already at the handle's Z height on its -X side.  This removes
        # the diagonal descent that struck the board in the previous run.
        initial_approach = pick_outside.copy()
        place_pre = place_grasp + self._approach_world * HORIZONTAL_APPROACH_DISTANCE

        # Go directly to the handle height on its -X side.  The final motion
        # around the handle is a pure world +X translation.
        self._targets = [
            initial_approach,
            pick_outside,
            pick_grasp,
            None,
            pick_vertical if VERTICAL_LIFT_TEST else pick_outside,
            pick_safe,
        ...
        print(
            "[serving-horizontal] ready "
            f"board={np.round(pick_center, 4)} "
            f"initial_-x={np.round(initial_approach, 4)} "
            f"lift_clearance={np.round(pick_safe, 4)} "
            f"vertical_test={np.round(pick_vertical, 4)} "
            f"handle_grasp={np.round(pick_grasp, 4)} "
    ...
    def step(self, articulation):
        ...
        target_orientation = HORIZONTAL_EE_ORIENTATION
        ...
        if self._settled_steps >= 15:
            ...
            message = (
                f"horizontal serving phase {self._phase} did not converge; "
            ...
            self.failed = True
            print(
                f"[serving-horizontal] STOPPED: {message}; "
