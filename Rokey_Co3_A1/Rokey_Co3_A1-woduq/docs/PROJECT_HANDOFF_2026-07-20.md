...
ROS topics verified on domain 101:
...
and saved successfully at 1280x960.
...
- Optional stationary `ParkingBrake` fixed joint
...
Table navigation is now the default, so the parking brake starts released. For
stationary arm tests, launch with:

```bash
MOBILE_DEMO_PARKED_HOLD=1 .../python.sh isaacpjt/mobile_manipulator_demo.py
```
