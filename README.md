...
flowchart LR
    Idle["0 대기"] -->|주문 접수| Spawn["2 음식 스폰"]
    Spawn --> Move["3 테이블 이동"]
    Move --> Serve["4 로봇팔 서빙"]
    Serve -->|손 감지| Paused["5 손 침입 정지"]
    Paused -->|손 빠짐| Serve
    Serve -->|서빙 완료| Return["1 주방 복귀"]
    Return --> Done["6 완료"]

    Spawn -.-> Failed["7 실패"]
    Move -.-> Failed
    Serve -.-> Failed
    Failed -.->|reset_fault| Idle

    classDef happy fill:#d1fae5,stroke:#047857,color:#022c22,rx:6,ry:6;
    classDef warn fill:#fef3c7,stroke:#b45309,color:#451a03,rx:6,ry:6;
    classDef bad fill:#fee2e2,stroke:#b91c1c,color:#450a0a,rx:6,ry:6;
    class Idle,Spawn,Move,Serve,Return,Done happy
    class Paused warn
    class Failed bad
```

Fleet Manager는 유휴 상태(`0` 또는 `6`)인 로봇 중 `robot1`을 우선 배정하고,
`robot1`이 주행 중이면 다음 주문을 `robot2`에 배정합니다. 손이 ROI에 5프레임
이상 감지되면 arm pause 명령(`99`)으로 정지하고, 손이 빠지면 resume
명령(`98`)으로 재개합니다. 손 안전 토픽이 서빙 중 2초 이상 끊기면 fail-safe로
상태 `7`이 됩니다.
...
jpeg_quality:=90
enable_orthogonal_routes:=false
use_sim_time:=true
```
...
```bash
./tools/t2_ros.sh enable_orthogonal_routes:=true
```
