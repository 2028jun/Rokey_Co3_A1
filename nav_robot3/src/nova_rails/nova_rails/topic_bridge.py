...
class TopicBridge(Node):
    def __init__(self) -> None:
        ...
        # Raw chassis odom origin at last /nova_carter/teleport (rebases /odom to ~identity).
        ...
        self.get_logger().info(
            "bridge: /chassis/odom -> /odom + TF odom->base_link, "
            f"scan -> /scan ({LIDAR_FRAME}), teleport rebases odom"
        )
...
def main() -> None:
    os.environ.setdefault("ROS_DOMAIN_ID", "104")
    rclpy.init()
    node = TopicBridge()
    try:
        rclpy.spin(node)
