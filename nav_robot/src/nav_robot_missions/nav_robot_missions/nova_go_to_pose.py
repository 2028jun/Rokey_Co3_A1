#!/usr/bin/env python3
"""Nova Carter kitchen -> table via Nav2 goToPose (sample_code/nav_to_pose.py style).

Uses sequential goToPose along the aisle so the planner stays in free space.
Initial pose matches Isaac spawn / AMCL (0.21, 5.25), not the kitchen dock
waypoint (0.21, 4.90).
"""
...
# Match isaacpjt/nav_nova_carter_demo.py SPAWN_* and amcl.initial_pose
...
def create_pose(navigator, x, y, yaw_rad):
    """x, y, yaw(라디안) → PoseStamped (waypoints.yaml 기준)."""
    pose = PoseStamped()
...
def route_kitchen_to_table(table: tuple[float, float, float]) -> list[tuple[float, float, float]]:
    """Aisle spine then branch to table dock."""
    tx, ty, tyaw = table
...
def wait_go_to_pose(nav: BasicNavigator, goal: PoseStamped, label: str) -> tuple[TaskResult, object]:
    nav.get_logger().info(
        f"Navigating to {label}: {goal.pose.position.x:.2f} {goal.pose.position.y:.2f}..."
    )
    nav.goToPose(goal)
    last_pose = None
    while not nav.isTaskComplete():
        ...
        time.sleep(1.0)
    return nav.getResult(), last_pose


def main(argv=None) -> int:
    os.environ.setdefault("ROS_DOMAIN_ID", "103")
    rclpy.init(args=None)
    nav = BasicNavigator("nova_go_to_pose")
    ...
    try:
        ...
        # sample_code: setInitialPose -> waitUntilNav2Active -> goToPose
        init_pose = create_pose(nav, *SPAWN_XY_YAW)
        nav.setInitialPose(init_pose)
        nav.waitUntilNav2Active()

        # Give AMCL/costmaps a moment after initial pose (scan bridge must be up).
        time.sleep(1.0)
        try:
        ...
        if direct:
            goals = [(table[0], table[1], table[2])]
        else:
            goals = route_kitchen_to_table(table)

        last_pose = None
        for i, (x, y, yaw) in enumerate(goals):
            label = f"wp{i + 1}/{len(goals)}"
            if i == len(goals) - 1:
                label = f"table_{table_id}"
            result, last_pose = wait_go_to_pose(
                nav, create_pose(nav, x, y, yaw), label
            )
