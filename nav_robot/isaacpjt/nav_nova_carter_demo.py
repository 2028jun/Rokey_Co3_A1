"""Isaac Sim 5.1 restaurant scene + Nova Carter (ROS USD) for Nav2 tests.

Loads the lightweight pizza restaurant and Nucleus Nova_Carter_ROS.usd.
Uses USD lidar/odom graphs, but drives wheels from Python /cmd_vel because the
built-in differential_controller often skips steps with Invalid deltaTime=0.

Topics (Isaac):
  /cmd_vel                 — Nav2 commands (Python applies to wheels)
  /chassis/odom            — USD odometry
  /front_2d_lidar/scan     — front 2D lidar
  TF: odom -> base_link (USD) + bridge republish for Nav2

Nav2 should launch with nova_carter_topic_bridge (/odom, /scan).

Run with Isaac's python:
  export NAV_ROBOT_ROS_DOMAIN_ID=103
  .../python.sh isaacpjt/nav_nova_carter_demo.py
"""
...
# rclpy is provided by isaacsim.ros2.bridge — enable first, then import.
...
# Nova Carter approx (Isaac Samples ROS2 robot).
WHEEL_RADIUS = 0.14
WHEEL_BASE = 0.413
...
def spawn_nova_carter() -> WheeledRobot:
    ...
    if not assets_root:
        raise RuntimeError(
            ...
            "Ensure Isaac assets are configured so Nova_Carter_ROS.usd is reachable."
        )
    ...
    print(
        ...
        f"yaw={SPAWN_YAW:.2f} prim={ROBOT_PRIM}",
        flush=True,
    )
    return robot
...
def configure_nova_ros_graphs() -> None:
    """Disable broken OG twist drive; keep lidar/odom publishers."""
    # Steer USD twist subscriber off /cmd_vel so Python owns drive.
    twist_topic_attrs = (
    ...
    for path in twist_topic_attrs:
        try:
            ...
            print(f"[nova] OG twist topic -> cmd_vel_og_disabled ({path})", flush=True)
        except Exception:
            pass
    ...
    for path in accel_attrs:
        try:
            ...
            print(f"[nova] set {path} = 0", flush=True)
        except Exception:
            pass


def create_clock_ros_graph() -> None:
    """Publish /clock so Nav2/RViz (use_sim_time) and map TF can advance.

    Without /clock, Fixed Frame 'map' never appears and the occupancy map
    looks blank in RViz even though /map is published.
    """
    ...
    print(f"[nova] /clock publisher ready (domain={domain})", flush=True)


class CmdVelDrive(Node):
    """Apply Nav2 /cmd_vel to Nova Carter wheels (bypass OG differential_controller)."""
    ...
    def __init__(self, robot: WheeledRobot):
        ...
        print(
            f"[nova] Python drive /cmd_vel R={WHEEL_RADIUS} base={WHEEL_BASE}",
            flush=True,
        )
    ...
    def apply(self) -> None:
        ...
        try:
            ...
            # Rare first-frame race before articulation is ready.
            if not hasattr(self, "_warned"):
                print(f"[warn] apply_wheel_actions: {exc}", flush=True)
                self._warned = True
...
def main() -> int:
    ...
    print(
        ...
        "drive=Python(/cmd_vel) /clock /chassis/odom /front_2d_lidar/scan "
        "(Nav2: nova_carter_topic_bridge)",
        flush=True,
    )
